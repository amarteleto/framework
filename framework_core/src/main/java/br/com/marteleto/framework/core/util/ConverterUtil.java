...
public class ConverterUtil implements Serializable {
    		...
    		if (methods != null && methods.length > 0) {
	            for (Method method : methods) {
	            	if (method.getParameterTypes() != null && method.getParameterTypes().length > 0 &&
	            		method.getReturnType() != null && method.getReturnType().getName() != null
	            	) {
    ...
    public static <T> T convert(Object from, Class<T> to) {
        ...
        ConverterUtil.processConverters();
        ...
        try {
            ...
            throw new UnsupportedOperationException("Cannot convert from " + from.getClass().getName() + " to " + to.getName() + ". Conversion failed with " + e.getMessage(), e);
        }
    }
